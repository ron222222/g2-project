...
class G2BinGraspController:
    ...
    def __init__(self):
        print("="*72)
        print("【版本】pnc-numeric-cycle-fixed / NO-SLAM-ODOM")
        print("="*72)
    ...
    def one_gripper(self,group,position):
    ...
    def gripper(self,position):
        try:
            left_ok=self.one_gripper("left_tool",position)
            time.sleep(0.10)
            right_ok=self.one_gripper("right_tool",position)
            time.sleep(0.20)
            return left_ok and right_ok
        except Exception as error:
            print(f"OmniPicker异常: {error}")
            return False
    ...
    def run(self):
        ...
        print("Step4 OmniPicker闭合并额外内收约1cm")
