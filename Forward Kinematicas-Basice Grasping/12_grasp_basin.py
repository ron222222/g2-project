...
class G2BinGraspController:
    ...
    # ==================== 预定义安全姿态 (基于V3避碰方案) ====================
    # 以下角度为示例，请根据现场实际调试后替换
    # 原则: 避免双臂侧平举(防止gripper_l_base_link与gripper_r_base_link碰撞)
    #       避免抱拳姿态(防止arm_l_link7与gripper_r_base_link碰撞)
    ...
    def pose_pregrasp(self):
        ...
        肩部轻微外展避免自碰撞
        """
        left  = [-0.2, -0.6,  0.3, -1.0,  0.2, 0.0, 0.0]
        right = [ 0.2, -0.6, -0.3, -1.0, -0.2, 0.0, 0.0]
        return left, right

    def pose_grasp(self):
        """
        抓取位姿: 双臂内收夹持
        需要根据方盆实际位置微调 joint1/2/3/4
        """
        left  = [-0.1, -0.4,  0.5, -0.8,  0.3, 0.0, 0.0]
        right = [ 0.1, -0.4, -0.5, -0.8, -0.3, 0.0, 0.0]
        return left, right

    def pose_lift(self):
        """抬起: 抓取后轻微上抬并回收"""
        left  = [-0.1, -0.5,  0.4, -1.0,  0.2, 0.0, 0.0]
        right = [ 0.1, -0.5, -0.4, -1.0, -0.2, 0.0, 0.0]
        return left, right

    def pose_place(self):
        """放置位姿: 面向右侧工作台"""
        left  = [ 0.3, -0.6,  0.0, -1.2, 0.0, 0.0, 0.0]
        right = [-0.2, -0.4,  0.2, -0.9, 0.1, 0.0, 0.0]
        return left, right
    ...
    def run_pipeline(self):
        ...
        # Step 7: 腰部旋转180° (idx05_body_joint5)
        print("\n【Step 7/9】腰部旋转180°")
        waist = self.get_waist_positions()
        # joint5 旋转，注意限位 [-3.0456, 3.0456]
        waist[4] += math.pi
        if waist[4] > 2.045599:
            waist[4] -= 2 * math.pi
