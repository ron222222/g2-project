...
- 启动时不再沿用任意当前躯干高度，而是先恢复到JSON中已标定的固定抓取高度
...
class G2BinGraspController:
    ...
    def save_current_waist_reference(self):
        """在官方方式已调到正确抓取高度后，保存该腰部姿态。只保存，不运动。"""
        ...
        print(f"末端高度: 左={left_z:.3f}m, 右={right_z:.3f}m")
        return True

    def load_safe_waist_reference(self):
        """读取已保存的抓取高度，禁止把运行时任意高度误当作安全目标。"""
        if not os.path.exists(self.CALIBRATION_FILE):
            print("未找到腰部高度标定文件，程序不会发送运动命令。")
            print("先用官方方式把机器人调到正确抓取高度，将CALIBRATE_ONLY设为True运行一次。")
            return False
        try:
