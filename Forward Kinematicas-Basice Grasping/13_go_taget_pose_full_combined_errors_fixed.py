...
版本：combined-errors-fixed（综合修复两次Slam odom null / GetOdomInfo failed）
...
- 综合修复两次报错：Slam odom is null、GetOdomInfo failed
- 底盘只使用relative_move目标+固定完成等待，不创建Slam对象，不读取里程计/任务状态
...
class G2BinGraspController:
    ...
    CHASSIS_SETTLE_TIME_S = 8.0
    CHASSIS_POST_STOP_WAIT_S = 0.8
    ...
    def __init__(self):
        ...
        print("【版本】combined-errors-fixed / NO-SLAM-ODOM")
        print("=" * 72)
        ...
        print("GDK、Robot、TF、PNC对象就绪")
        print("末端配置: left_tool/right_tool + omnipicker")
        print("底盘模式: PNC.relative_move，无SLAM里程计依赖")
    ...
    def move_chassis_relative(self, distance_m):
        """
        当前部署的Slam里程计为空，因此只使用PNC relative_move。
        本方法不创建Slam对象、不调用get_odom_info，也不依赖get_task_state。
        distance_m > 0前进，distance_m < 0后退。
        """
        ...
        try:
            ...
            print(f"底盘{direction}请求已发送，等待{self.CHASSIS_SETTLE_TIME_S:.1f}s")
            time.sleep(self.CHASSIS_SETTLE_TIME_S)

            # 发送零速度作为明确停止保护，然后给控制器短暂稳定时间。
            self.stop_chassis()
            time.sleep(self.CHASSIS_POST_STOP_WAIT_S)
            print(f"底盘{direction}阶段结束")
            return True
        except Exception as e:
            self.stop_chassis()
