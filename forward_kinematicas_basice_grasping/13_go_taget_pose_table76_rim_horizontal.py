...
class G2BinGraspController:
    ...
    # 桌面抓取逆运动学种子（单位：rad）
    # 实测桌面0.76m，抓取盆上边沿（约0.845m），盆宽0.32m，盆中心位于base_link前方约0.55m。
    # 目标夹爪中心：预抓取(0.55, ±0.265, 0.945)，抓取(0.55, ±0.195, 0.845)，
    # 抬升(0.55, ±0.195, 0.995)。左右夹爪严格水平相向。
    WAIST_HOME = [-0.1026, 0.3217, 0.1123, 0.0, 0.0]
    WAIST_PREGRASP = [-0.1026, 0.3217, 0.1123, 0.0, 0.0]
    WAIST_GRASP = [-0.0557, 0.3476, 0.1107, 0.0, 0.0]
    WAIST_LIFT = [-0.1055, 0.2543, 0.0123, 0.0, 0.0]

    def pose_home(self):
        """桌面任务安全准备姿态。"""
        left = [0.2267, -0.4420, 0.5909, -1.2322, 1.3285, -0.0429, 1.5350]
        right = [-0.2267, -0.4420, -0.5909, -1.2322, -1.3285, -0.0429, -1.5350]
        return left, right

    def pose_pregrasp(self):
        """夹爪中心约为(0.55, ±0.265, 0.945)，水平相向。"""
        left = [0.2267, -0.4420, 0.5909, -1.2322, 1.3285, -0.0429, 1.5350]
        right = [-0.2267, -0.4420, -0.5909, -1.2322, -1.3285, -0.0429, -1.5350]
        return left, right

    def pose_grasp(self):
        """夹爪中心约为(0.55, ±0.195, 0.845)，抓取76cm桌面上的85mm盆上边沿。"""
        left = [0.4110, -0.6219, 0.6054, -1.2139, 1.2238, -0.0248, 1.4051]
        right = [-0.4110, -0.6219, -0.6054, -1.2139, -1.2238, -0.0248, -1.4051]
        return left, right

    def pose_lift(self):
        """保持横向夹持间距，将夹爪中心从盆沿抬升到约0.995m。"""
        left = [0.2753, -0.6491, 0.5385, -1.1503, 1.2477, -0.0285, 1.4217]
        right = [-0.2753, -0.6491, -0.5385, -1.1503, -1.2477, -0.0285, -1.4217]
        return left, right

    def pose_place(self):
        """腰部转向后，再降回与抓取阶段相同的相对高度和夹持宽度。"""
        left = [0.4110, -0.6219, 0.6054, -1.2139, 1.2238, -0.0248, 1.4051]
        right = [-0.4110, -0.6219, -0.6054, -1.2139, -1.2238, -0.0248, -1.4051]
        return left, right
    ...
    def run_pipeline(self):
        ...
        # Step 1: 安全归位
        print("\n【Step 1/9】安全归位")
        if not self.move_waist(self.WAIST_HOME, velocity=0.15):
            return False
        l, r = self.pose_home()
        if not self.move_arms(l, r, velocity=0.20):
            return False
        time.sleep(0.5)
        ...
        # Step 3: 预抓取姿态 (双臂外展避碰)
        print("\n【Step 3/9】预抓取姿态（夹爪水平相向）")
        if not self.move_waist(self.WAIST_PREGRASP, velocity=0.12):
            return False
        ...
        # Step 4: 抓取姿态
        print("\n【Step 4/9】抓取姿态")
        if not self.move_waist(self.WAIST_GRASP, velocity=0.10):
            return False
        l, r = self.pose_grasp()
        if not self.move_arms(l, r, velocity=0.10):
            return False
        time.sleep(0.3)
        ...
        # Step 6: 抬起
        print("\n【Step 6/9】抬起方盆")
        if not self.move_waist(self.WAIST_LIFT, velocity=0.10):
            return False
        l, r = self.pose_lift()
        if not self.move_arms(l, r, velocity=0.10):
            return False
