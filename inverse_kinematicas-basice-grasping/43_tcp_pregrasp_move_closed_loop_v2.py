...
# -*- coding: utf-8 -*-
"""
43_tcp_pregrasp_move_closed_loop_v2.py
...
REPORT_FILE = "tcp_pregrasp_move_closed_loop_v2_report.json"
...
ROTATION_CLEARANCE_EXTRA_M = 0.050
...
def stage_pose_check(tf_api, target_position, target_quaternion, stage_name,
                     position_tolerance_m=0.020, orientation_tolerance_deg=5.0):
    measured = wait_for_pose(tf_api, RIGHT_FRAME)
    position_error = distance(measured.position, target_position)
    orientation_error = quaternion_angle_error_deg(
        measured.orientation, target_quaternion
    )
    print(
        f"[{stage_name} 验证] 位置误差={position_error:.4f} m, "
        f"姿态误差={orientation_error:.2f} degree"
    )
    return measured, position_error, orientation_error


def require_stage_reached(report, stage_name, measured, position_error,
                          orientation_error, position_tolerance_m,
                          orientation_tolerance_deg):
    record = {
        "stage": stage_name,
        "measured_position": measured.position,
        "measured_orientation": measured.orientation,
        "position_error_m": round(position_error, 6),
        "orientation_error_deg": round(orientation_error, 4),
        "position_tolerance_m": position_tolerance_m,
        "orientation_tolerance_deg": orientation_tolerance_deg,
    }
    report.setdefault("stage_validations", []).append(record)
    save_report(report)
    if position_error > position_tolerance_m:
        report["status"] = f"STOPPED_{stage_name.upper()}_POSITION"
        save_report(report)
        raise RuntimeError(f"{stage_name} 未到位，停止后续阶段")
    if orientation_error > orientation_tolerance_deg:
        report["status"] = f"STOPPED_{stage_name.upper()}_ORIENTATION"
        save_report(report)
        raise RuntimeError(f"{stage_name} 姿态未到位，停止后续阶段")
...
def main():
    ...
    report = {
        "program": "43_tcp_pregrasp_move_closed_loop_v2.py",
    ...
    try:
        ...
        print("=" * 78)
        print("43_tcp_pregrasp_move_closed_loop_v2.py")
        ...
        measured, pos_err, ori_err = stage_pose_check(
            tf_api, clearance_start_position, end_initial.orientation,
            "stage1_raise"
        )
        require_stage_reached(
            report, "stage1_raise", measured, pos_err, ori_err,
            0.020, 5.0
        )
        ...
        time.sleep(TF_SETTLE_SECONDS)
        measured, pos_err, ori_err = stage_pose_check(
            tf_api, clearance_start_position, end_target_quaternion,
            "stage2_rotate", position_tolerance_m=0.025,
            orientation_tolerance_deg=5.0
        )
        require_stage_reached(
            report, "stage2_rotate", measured, pos_err, ori_err,
            0.025, 5.0
        )
        ...
        time.sleep(TF_SETTLE_SECONDS)
        measured, pos_err, ori_err = stage_pose_check(
            tf_api, clearance_target_position, end_target_quaternion,
            "stage3_translate"
        )
        require_stage_reached(
            report, "stage3_translate", measured, pos_err, ori_err,
            0.020, 5.0
        )
