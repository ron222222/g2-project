...
# -*- coding: utf-8 -*-
"""
60_g2_yolo_live_axis_y_grasp_down3cm.py
...
# OpenCV Qt字体路径修复。必须在import cv2之前设置。
...
REPORT_FILE = "g2_yolo_live_axis_y_grasp_down3cm_report.json"
ANGLE_DEBUG_IMAGE = "g2_product_angle_axis_y_debug.jpg"
LIVE_WINDOW_NAME = "G2 YOLO Live"
LIVE_LATEST_IMAGE = "g2_yolo_axis_y_live_latest.jpg"
SHOW_YOLO_WINDOW = True
SAVE_LATEST_FRAME = True
DISPLAY_SCALE = 1.0
NO_DETECTION_SLEEP_S = 0.03
WINDOW_WAIT_KEY_MS = 10
...
MAX_YAW_PER_STAGE_DEG = 15.0
ROTATION_INTERPOLATION_STEP_DEG = 1.0
...
def rotate_tcp_yaw_staged(
    ...
    for stage_index in range(1, stage_count+1):
        ...
        for interpolation_index in range(1, interpolation_count+1):
            ...
            # 每个插值点都根据阶段开始时的实际TCP重新计算End，减少TCP漂移。
...
def main() -> None:
    report = {
        "program": "60_g2_yolo_live_axis_y_grasp_down3cm.py",
    ...
    try:
        ...
        print("="*78)
        print("60_g2_yolo_live_axis_y_grasp_down3cm.py")
