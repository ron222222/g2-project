...
def main():
    report={
        "program":"86_g2_pick_three_cycles.py",
    ...
    try:
        ...
        print("="*78)
        print("86_g2_pick_three_cycles.py")
        ...
        for cycle_index in range(1,CYCLE_COUNT+1):
            ...
            if det["product_z_range_m"]>MAX_PRODUCT_Z_RANGE_M:
                raise RuntimeError(
                    f"第{cycle_index}轮产品Z多帧不稳定: "
                    f"极差={det['product_z_range_m']:.4f}m"
                )
            ...
            if (
                PRODUCT_HEIGHT_VALID_MIN_M
                <= measured_product_height
                <= PRODUCT_HEIGHT_VALID_MAX_M
            ):
            ...
            yaw=det["yaw_rad"]
            target_q=vertical_tool_orientation(
                yaw+math.radians(GRIPPER_YAW_OFFSET_DEG)
            )
            ...
            if cycle_index<CYCLE_COUNT:
                if WAIT_FOR_PRODUCT_CONFIRMATION:
                    ...
                    print(
                        f"等待{INTER_CYCLE_WAIT_S:.1f}s放置下一件产品..."
                    )
